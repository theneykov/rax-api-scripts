...
print "You're about to create ", srv_count, " Cloud Servers and attach them behind a newly created Cloud Load Balancer."
print 
...
if answer == "y":
		...
		for check in server_matrix:
			if len(check[3]) < 1 or len(check[4]) < 1:
				received_ips = False
	print "['                uuid                ', ' name ', 'root password', '  Public IP  ', '  Service Net  ']"        
	...
	for details in lb:
        	print
        	print "CLB: ", details.id, " ", details.name, " ", details.status, " ", details.virtual_ips, " ", details.port, " ", details.protocol
        	print "Nodes: "
        	for n in details.nodes:
                	print "      \- ", n.id, " ", n.condition, " ", n.status, " ", n.type, " ", str(n.address).strip() + ":" + str(n.port).strip()
